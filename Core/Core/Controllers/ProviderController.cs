...
namespace WebApplication1.Controllers
{
    public class ProviderController : ApiController
    {
        ...
        {
            string action = "UPDATE EPROVIDER SET P_Name = '" + provider.P_Name + "',P_LName = '" + provider.P_LName+"',P_Address = '" + provider.P_Address+ "',P_Date = CAST('" + provider.P_Date.ToString("yyyy-MM-dd") + "' AS DATETIME)" + ",P_Status = '"+provider.P_Status + "' WHERE P_ID =" + provider.P_ID;
        ...
        public JsonResult<List<Provider>> Get(string attribute, string id)
        {
            ...
            while (reader.Read())
            {
                ...
                emp.P_Status = reader.GetValue(5).ToString();
                values.Add(emp);
            }           
            myConnection.Close();
            return Json(values);
